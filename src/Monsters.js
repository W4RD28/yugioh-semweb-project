...
const monsterColumnData = [...defaultColumnData]
monsterColumnData.splice(3, 0, 
    {"label": "ATK", "selector": "atk"}, 
    {"label": "DEF", "selector": "def"},
    {"label": "Type", "selector": "race"},
    {"label": "Attribute", "selector": "attribute"})

const monsterColumnSelectors = [...defaultColumnSelectors]
monsterColumnSelectors.splice(3, 0, "atk", "def", "race", "attribute")
...
const monsterQuery = `SELECT ?type ?name ?id ?atk ?def ?race ?attribute ?category ?text ?image {
      ?subject a ?type ;
      :id ?id ;
      :name ?name ;
      :atk ?atk ;
...
class Monsters extends Component {
  ...
  refreshData() {
      ...
      query.execute(conn, dbName, monsterQuery).then(res => {
          if (!res.ok) {
          this.setState({
  ...
  getBindingsFormattedForTable(bindings) {
    ...
    renderRowForBinding(binding, index) {
        ...
        <TableRow key={binding.id}>
            {monsterColumnSelectors.map(selector => (
                selector === "image" ?
    ...
    render() {
        const columnHeaders = monsterColumnData.map(({ label }) => <TableCell key={label}>{label}</TableCell>)
        const { dataState, data } = this.state;
        const isLoading = dataState === TableDataAvailabilityStatus.LOADING;

        return (
            <div className="Monsters" style={styles.appInnerContainer}>
                <CssBaseline />
